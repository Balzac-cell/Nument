...
def app():
    ...
    with col1:
        ...
        st.markdown("---")
        st.header("Date de Naissance")

        try:
            st.image("app4/assets/Split/DateDeNaissance/Split_DateDeNaissance.jpg", use_container_width=False, width=175)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
        st.markdown("")
        DateNaissance = st.text_input("Saisissez la donnée encadrée en vert et appuyez sur entrer", key="DateNaissance", autocomplete="off")

        if DateNaissance:

            # initialisation
            update_payload = {"DateNaissance": DateNaissance}
            ...
            try:
                ...
                if response.status_code == 204:  # 204 = Modification réussie
                    ...
                    st.session_state["page"] = "page11"  # Rediriger vers page
                    st.rerun()
                else:
    ...
    with col2:
        try:
            ...
            st.image("app4/assets/Split/DateDeNaissance/DateDeNaissance.jpg", use_container_width=True
)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
