...
def app():
    ...
    with col1:
        ...
        st.markdown("---")
        st.header("Date de délivrance")

        try:
            st.image("app4/assets/Split/DateDeliv/Split_DateDeliv.jpg", use_container_width=False, width=175)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
        st.markdown("")
        DateDeliv = st.text_input("Saisissez la donnée encadrée en vert et appuyez sur entrer", key="DateDeliv", autocomplete="off")

        if DateDeliv:

            # initialisation
            update_payload = {"DateDeliv": DateDeliv}
            ...
            try:
                ...
                if response.status_code == 204:  # 204 = Modification réussie
                    ...
                    st.session_state["page"] = "page15"  # Rediriger vers page
                    st.rerun()
                else:
    ...
    with col2:
        try:
            ...
            st.image("app4/assets/Split/DateDeliv/DateDeliv.jpg", use_container_width=True
)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
