...
def app():
    ...
    with col1:
        ...
        st.markdown("---")
        st.header("Numéro du document")

        try:
            st.image("app4/assets/Split/NumDoc/Split_NumDoc.jpg", use_container_width=False, width=175)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
        st.markdown("")
        NumDoc = st.text_input("Saisissez la donnée encadrée en vert et appuyez sur entrer", key="NumDoc", autocomplete="off")

        if NumDoc:

            # initialisation
            update_payload = {"NumDoc": NumDoc}
            ...
            try:
                ...
                if response.status_code == 204:  # 204 = Modification réussie
                    ...
                    st.session_state["page"] = "page13"  # Rediriger vers page
                    st.rerun()
                else:
    ...
    with col2:
        try:
            ...
            st.image("app4/assets/Split/NumDoc/NumDoc.jpg", use_container_width=True
)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
