...
def app():
    ...
    with col1:
        ...
        st.markdown("---")
        st.header("Code Postal")

        try:
            st.image("app4/assets/Split/Ville/Split_Ville.jpg", use_container_width=False, width=175)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
        st.markdown("")
        CP = st.text_input("Saisissez le code postal", key="CP", autocomplete="off")
        st.markdown("")
        st.header("Ville")
        Ville = st.text_input("Saisissez la ville et appuyez sur entrer", key="Ville", autocomplete="off")

        if Ville and CP:

            # initialisation
            update_payload = {"Ville": Ville, "CP": CP}
            ...
            try:
                ...
                if response.status_code == 204:  # 204 = Modification réussie
                    ...
                    st.session_state["page"] = "page16"  # Rediriger vers page
                    st.rerun()
                else:
    ...
    with col2:
        try:
            ...
            st.image("app4/assets/Split/Ville/Ville.jpg", use_container_width=True
)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
