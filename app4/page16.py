...
def app():
    ...
    with col1:
        ...
        try:
            st.image("app4/assets/Split/Pays/Split_Pays.jpg", use_container_width=False, width=175)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
        st.markdown("")
        Pays = st.text_input("Saisissez la donnée encadrée en vert et appuyez sur entrer", key="Pays", autocomplete="off")

        if Pays:

            # initialisation
            update_payload = {"Pays": Pays}
            ...
            try:
                ...
                if response.status_code == 204:  # 204 = Modification réussie
                    ...
                    st.session_state["page"] = "page17"  # Rediriger vers page
                    st.rerun()
                else:
    ...
    with col2:
        try:
            ...
            st.image("app4/assets/Split/Pays/Pays.jpg", use_container_width=True
)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
