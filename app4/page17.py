...
def app():
    ...
    with col1:
        ...
        st.markdown("---")
        st.header("Première ligne MRZ")

        try:
            st.image("app4/assets/Split/MRZ1/Split_MRZ1.jpg", use_container_width=False, width=175)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
        st.markdown("")
        MRZ1 = st.text_input("Saisissez la donnée encadrée en vert et appuyez sur entrer", key="MRZ1", autocomplete="off")

        if MRZ1:

            # initialisation
            update_payload = {"MRZ1": MRZ1}
            ...
            try:
                ...
                if response.status_code == 204:  # 204 = Modification réussie
                    ...
                    st.session_state["page"] = "FinFormulaireComplet"  # Rediriger vers page
                    st.rerun()
                else:
    ...
    with col2:
        try:
            ...
            st.image("app4/assets/Split/MRZ1/MRZ1.jpg", use_container_width=True
)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
