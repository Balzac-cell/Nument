...
def app():
    ...
    with col1:
        ...
        try:
            st.image("app4/assets/Split/Prenom/Split_Penom.jpg", use_container_width=False, width=175)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
        st.markdown("")
        Prenoms_1 = st.text_input("Saisissez la donnée encadrée en vert et appuyez sur entrer", key="Prenoms_1", autocomplete="off")

        if Prenoms_1:

            # initialisation
            update_payload = {"Prenoms_1": Prenoms_1}
            ...
            try:
                ...
                if response.status_code == 204:  # 204 = Modification réussie
                    ...
                    st.session_state["page"] = "page9"  # Rediriger vers page
                    st.rerun()
                else:
    ...
    with col2:
        try:
            ...
            st.image("app4/assets/Split/Prenom/Prenom.jpg", use_container_width=True
)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
