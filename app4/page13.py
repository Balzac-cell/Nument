...
def app():
    ...
    with col1:
        ...
        st.markdown("---")
        st.header("Date d'expiration")

        try:
            st.image("app4/assets/Split/DateExpi/Split_DateExpi.jpg", use_container_width=False, width=175)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
        st.markdown("")
        DateExpir = st.text_input("Saisissez la donnée encadrée en vert et appuyez sur entrer", key="DateExpir", autocomplete="off")

        if DateExpir:

            # initialisation
            update_payload = {"DateExpir": DateExpir}
            ...
            try:
                ...
                if response.status_code == 204:  # 204 = Modification réussie
                    ...
                    st.session_state["page"] = "page14"  # Rediriger vers page
                    st.rerun()
                else:
    ...
    with col2:
        try:
            ...
            st.image("app4/assets/Split/DateExpi/DateExpi.jpg", use_container_width=True
)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
