...
def app():
    ...
    with col1:
        ...
        try:
            st.image("app4/assets/Split/Au/Au.jpg", use_container_width=False, width=250)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
        st.markdown("")
        au = st.text_input("Saisissez la donnée encadrée en vert et appuyez sur entrer", key="au", autocomplete="off")

        if au:

            # initialisation
            update_payload = {"au": au}
            ...
            try:
                ...
                if response.status_code == 204:  # 204 = Modification réussie
                    ...
                    st.session_state["page"] = "page5"  # Rediriger vers page4
                    st.rerun()
                else:
    ...
    with col2:
        try:
            ...
            st.image("app4/assets/Split/Au/SplitAu.jpg", caption="Bloc 4 sur 6", use_container_width=True
)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
