...
def app():
    ...
    with col1:
        ...
        st.markdown("---")
        st.header("Lieu de Naissance")

        try:
            st.image("app4/assets/Split/LieuDeNaiss/Split_LieuDeNaiss.jpg", use_container_width=False, width=175)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
        st.markdown("")
        LieuNaissance = st.text_input("Saisissez la donnée encadrée en vert et appuyez sur entrer", key="LieuNaissance", autocomplete="off")

        if LieuNaissance:

            # initialisation
            update_payload = {"LieuNaissance": LieuNaissance}
            ...
            try:
                ...
                if response.status_code == 204:  # 204 = Modification réussie
                    ...
                    st.session_state["page"] = "page12"  # Rediriger vers page
                    st.rerun()
                else:
    ...
    with col2:
        try:
            ...
            st.image("app4/assets/Split/LieuDeNaiss/LieuDeNaiss.jpg", use_container_width=True
)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
