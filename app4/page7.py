...
def app():
    ...
    with col1:
        ...
        st.markdown("---")
        st.header("Nom de Famille")

        try:
            st.image("app4/assets/Split/NomDeFamille/Split_NomDeFamille.jpg", use_container_width=False, width=175)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
        st.markdown("")
        NomDeFamille = st.text_input("Saisissez la donnée encadrée en vert et appuyez sur entrer", key="NomDeFamille", autocomplete="off")

        if NomDeFamille:

            # initialisation
            update_payload = {"NomDeFamille": NomDeFamille}
            ...
            try:
                ...
                if response.status_code == 204:  # 204 = Modification réussie
                    ...
                    st.session_state["page"] = "page8"  # Rediriger vers page6
                    st.rerun()
                else:
    ...
    with col2:
        try:
            ...
            st.image("app4/assets/Split/NomDeFamille/NomDeFamille.jpg", use_container_width=True
)
        except FileNotFoundError:
            st.warning("Image non trouvée.")
